...
class Perceptron(object):
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        """YOUR CODE"""
        newValues = inActs[:]
        newValues.insert(0, 1)
        return self.sigmoidDeriv(self.getWeightedSum(newValues))
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        Inputs:
            inActs (list<float/int>): input values, not including biasd
        ...
        for indx in range(len(self.weights)):
            #weight = self.weights[indx]
            totalModification += abs(alpha * delta * newValues[indx])
            self.weights[indx] += alpha * delta * newValues[indx]
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.hiddenLayers:
            temp = []
            currentValues = out[currentIndex]
            for perceptron in layer:
                #print "perceptron of the layer: " + str(len(layer))
                temp.append(perceptron.sigmoidActivation(currentValues))
            out.append(temp)
            #print "out length: " + str(len(out[currentIndex + 1]))
            currentIndex += 1
        currentValues = out[currentIndex]
        temp = []
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    for example in examplesTest:
        out = nnet.feedForward(example[0])[-1]
        #print "result: " + str(out)
        out = map(lambda x: round(x), out)
        #print "out after: " + str(out)
        #print "example out" + str(example[1])
        if out == example[1]:
            testCorrect += 1
        else:
            testError += 1       
